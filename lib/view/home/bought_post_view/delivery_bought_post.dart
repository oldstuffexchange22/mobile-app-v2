...
class DeliveryBoughtPost extends StatefulWidget {
  const DeliveryBoughtPost({Key? key}) : super(key: key);

  @override
  State<DeliveryBoughtPost> createState() => _DeliveryBoughtPostState();
}

class _DeliveryBoughtPostState extends State<DeliveryBoughtPost> {
  ...
  void initState() {
    ...
    postBoughtProvider.getDeliveryPost();
    emptyWidget = const CenterRefresh();
    _timer = Timer(const Duration(milliseconds: 7000), () {
      if (postBoughtProvider.deliveryPosts.isEmpty && mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    List<Post> posts = postBoughtProvider.deliveryPosts;
    return Scaffold(
      body: RefreshIndicator(
        onRefresh: () => postBoughtProvider.getDeliveryPost(),
        child: posts.isEmpty
